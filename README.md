...
当前版本 **0.3.2** · Windows · Python 3.11+ · SQLite schema v4 · 162 项本地测试通过。
...
- 选中译文中的一段，或保留当前疑问，再点「记住这个疑问」，可编辑并保存到当前项目；不会自动把整篇回答写入长期记忆。
...
学习记忆保存在 **data/study.db 的 memories 表**；使用 --home 时位于该目录下的 data/study.db，例如 D:\MyStudyData\data\study.db。附带截图保存在相邻 data/screenshots/，升级备份在 data/backups/。

**阅读历史**是自动保留的最近完成回答，最多 60 条；**学习记忆**只保存你明确编辑确认的疑问或笔记，没有随阅读历史清理的数量限制。待解决只表示尚未标记解决，不表示系统判断你没有掌握知识。

- **保存**：回答完成后点底部“记住这个疑问”，优先带入选中文字、当前问题或最近已发送的问题，不自动总结整篇回答。确认所属项目、正文，可选概念和别名、状态、已知页码／章节，再保存。默认同时保留现存截图。
- **管理**：点“项目记忆”，可查看正文和时间、编辑、标记解决／重新打开、查看来源或删除。重启后重新选择原项目即可找到。删除记忆不删除阅读记录及已保留截图。
- **交给模型的条件**：勾选“本次使用项目记忆”，当前问题、辅助文字、Topic 或指定概念存在可靠匹配，且记忆属于当前项目并为待解决；最多 5 条，正文及来源等序列化总长最多 6000 字符。已解决默认排除，不相关不凑数，不跨项目或自动携带全局记忆。
- **纯截图**：没有文字线索时点“指定概念／记忆”，填写例如 gm／跨导，或勾选相关待解决疑问。该操作不识别图片、不调用模型；选择沿用同一项目与资料的后续请求，可清空。开启自动翻译时可在下一次截图前指定。
- **关闭使用**：取消勾选后，新请求不携带长期记忆，保留当前问题和必要的普通追问内容；旧回答若依赖已停用或改动的记忆，不再自动携带该回答正文。该开关默认开启，重启后恢复默认；它与持久保存的自动发送设置相互独立。
- **检查本次**：回答上方显示“本次使用 N 条学习记忆”或“本次未使用历史学习记忆”。点击查看发送时的正文、概念和来源；后续数据库编辑不会改写旧记录。连接尚未确认提交时不会声称已经使用，老版本回答缺少记录时明确标为未知。更多中的详情仅是待发送候选。
- **来源缺失**：原阅读结果可能被清理，PNG 也可能被外部移动。查看来源会提示缺失，记忆正文仍可使用。没有已知页码或章节时显示未知，不猜测。

“使用”表示本次请求确实携带了背景，不保证模型一定引用或引用正确。概念别名沿用共享术语表，但记忆正文严格按项目选择。当前采用简单词语／概念匹配，不具备语义检索；可通过手动选择补充漏检。

本阶段核查、迁移取舍、自动化覆盖及逐步手动验收：[项目记忆实现与验证](docs/V0.3.2_MEMORY.md)；[本阶段需求](PROJECT_TASK_V0.3.2.md)。
...
数据库增量升级到 **schema v4**，在原有记忆中补充状态和来源，在 reading_results 中增加发送时的记忆记录；已有表和用户数据保留。迁移前自动在线备份，事务失败回滚，不删除重建数据库。旧程序无法打开更高版本的数据，请使用对应备份回退，勿覆盖运行中的数据库。
...
- 重启后需重新选择项目与资料，上一 Session 不自动恢复。学习记忆可在“项目记忆”中查看、编辑、解决和删除；暂未提供长期图片图库。
